...
% Double filet a coins arrondis (bleu ENSAE + cyan) avec petits accents
% orange aux quatre angles.
\usepackage{eso-pic}
\AddToShipoutPictureBG{%
  \AtPageLowerLeft{%
    \begin{tikzpicture}[overlay]
      \coordinate (SW) at (14pt,14pt);
      \coordinate (NE) at ($(\paperwidth,\paperheight)-(14pt,14pt)$);
      \coordinate (sw) at (19pt,19pt);
      \coordinate (ne) at ($(\paperwidth,\paperheight)-(19pt,19pt)$);
      % filet bleu exterieur (coins arrondis)
      \draw[ensaeblue, line width=1.8pt, rounded corners=7pt] (SW) rectangle (NE);
      % filet cyan interieur
      \draw[ensaecyan, line width=0.7pt, rounded corners=5pt] (sw) rectangle (ne);
      % accents d'angle
      \foreach \c in {(SW),(NE),($(NE|-SW)$),($(SW|-NE)$)}
        \fill[ensaeorange] \c circle (2.4pt);
    \end{tikzpicture}%
  }%
}
...
\begin{appendices}
  ...
  \input{annexes/annexe_E}
  \input{annexes/annexe_F}
  \input{annexes/annexe_G}
\end{appendices}
