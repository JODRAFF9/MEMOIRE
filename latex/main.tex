...
% En-tete de section : pastille numerotee a droite, filet epais, titre
% centre, filet fin. #1 = titre, #2 = numero (vide pour une section non
% numerotee, la pastille n'est alors pas tracee).
...
\newcommand{\ensaetetesection}[2]{%
  ...
  \begin{tikzpicture}
    \ifx\relax#2\relax
      % Section non numerotee : sans pastille, le filet occupe toute la
      % largeur, faute de quoi la droite resterait vide.
      \fill[ensaeblue] (0,-3pt) rectangle (\textwidth,3pt);
    \else
      % Le filet se deroule depuis le bas de la pastille, comme un tapis
      % depuis son rouleau : il court jusqu'au centre du cercle, dont le
      % bord inferieur affleure son epaisseur. Le cercle etant trace
      % ensuite, l'extremite du filet disparait sous le rouleau.
      \fill[ensaeblue] (0,-3pt) rectangle (0.925\textwidth,3pt);
      \node[circle, fill=ensaeblue, text=white, minimum size=2.1cm,
            inner sep=0pt, font=\fontsize{42}{42}\selectfont]
        at (0.925\textwidth,0.90cm) {#2};
    \fi
  \end{tikzpicture}\par
  \vspace{0.32cm}
  {\centering\color{ensaeblue}\Large\bfseries #1\par}
  \vspace{0.32cm}
  \noindent{\color{ensaeblue!35}\rule{\textwidth}{0.6pt}}\par
  \endgroup
  \vspace{0.10cm}
}
