...
\subsection{Test d'équilibre des covariables (biais standardisé)}

Avant et après appariement, l'équilibre de chaque covariable $X$ entre
entrants ($D=1$) et témoins ($D=0$) est mesuré par la différence
...
\subsection{Score de propension (probit)}
...
noyau d'Epanechnikov et rayon (\textit{caliper}, $\epsilon = 0{,}05$, sans
...
\subsection{Estimateur de double différence (DD)}
...
\subsection{Test placebo (permutation)}

Pour vérifier l'absence d'effet artefactuel, un faux statut de traitement est
attribué aléatoirement à la même proportion de ménages jamais bénéficiaires,
...
\subsection{Test d'hétérogénéité (égalité des ATT)}
