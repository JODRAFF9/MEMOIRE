\section{Reproduction des taux de privation de l'ANSD}
\label{ann:E}

L'ANSD publie, dans son actualisation N-MODA 2024, la prévalence des
privations par indicateur et groupe d'âge sur l'EHCVM 2018/19. Le
tableau~\ref{tab:validation_ansd} confronte ces valeurs à celles obtenues avec
l'opérationnalisation retenue dans ce mémoire.

\begin{table}[H]
  \centering
  \caption{Comparaison de la prévalence des privations par indicateur : ce mémoire vs ANSD (EHCVM 2018, \%)}
  \label{tab:validation_ansd}
  \begin{threeparttable}
  \small
  \zebre
  \begin{tabular}{|l|c|c|c|c|c|c|}
    \hline
\entete Indicateur & \multicolumn{3}{|c|}{Ce mémoire} & \multicolumn{3}{|c|}{ANSD (MODA 2024)} \\ \hline
\entete & 0-4 & 5-14 & 15-17 & 0-4 & 5-14 & 15-17 \\
    \hline
    Type de toilettes                & 40,3 & 39,1 & 34,1 & 40,3 & 39,8 & 34,1 \\ \hline
    Partage des toilettes            & 20,4 & 19,4 & 18,6 & 20,4 & 19,4 & 18,6 \\ \hline
    Source d'eau non améliorée       & 20,7 & 19,2 & 16,1 & 12,0 & 10,7 & 8,5  \\ \hline
    Temps d'accès à l'eau $\geq 30$ min & 17,8 & 16,8 & 13,6 & 17,7 & 16,4 & 13,9 \\ \hline
    Débarras des ordures             & 59,3 & 58,2 & 53,0 & 59,3 & 58,2 & 53,0 \\ \hline
    Surpeuplement\tnote{a}           & 26,5 & 25,6 & 23,5 & n.d. & n.d. & 11,4 \\ \hline
    Insécurité alimentaire           & 45,0 & 46,5 & 44,9 & 45,0 & 46,5 & 44,9 \\ \hline
    Combustible solide               & 92,3 & 92,3 & 91,5 & 92,3 & 92,3 & 91,5 \\ \hline
    Accès santé (pas à pied)         & 48,0 & 47,2 & 44,0 & 79,3 & 78,5 & 75,2 \\ \hline
    Acte de naissance                & 31,8 & 30,0 & -    & 31,8 & 30,1 & -    \\ \hline
    Travail des enfants              & -    & 41,8 & -    & -    & 43,0 & -    \\ \hline
    Séparation parentale\tnote{b}    & 36,5 & 43,0 & 52,8 & n.d. & n.d. & n.d. \\ \hline
    Non-scolarisation                & -    & 45,3 & -    & -    & 45,3 & -    \\ \hline
    Illettrisme                      & -    & -    & 28,1 & -    & -    & 28,1 \\
    \hline
  \end{tabular}
  \begin{tablenotes}
    \footnotesize
    \item[a] Surpeuplement~: calculé sur le nombre total de pièces (la variable
    « pièces pour dormir » de l'ANSD est absente de l'EHCVM), d'où un proxy
    non directement comparable à la valeur ANSD.
    \item[b] Séparation parentale (ne vit pas avec ses deux parents
    biologiques)~: retenu dans la dimension Protection~; l'ANSD ne publie pas
    de taux par groupe d'âge pour cet indicateur.
  \end{tablenotes}
  \end{threeparttable}

  {\footnotesize \textit{Source :} Calcul de l'auteur, EHCVM~; ANSD/UNICEF (MODA 2024) pour la colonne de comparaison.}
\end{table}

Une fois les estimations pondérées par les poids de sondage de l'EHCVM
(comme dans le calcul officiel), la plupart des indicateurs reproduisent les
taux de l'ANSD de façon très étroite~: plusieurs coïncident au dixième de
point (insécurité alimentaire, combustible solide, illettrisme, type de
toilettes et partage des toilettes), les écarts résiduels restant
inférieurs à 3 points. Le partage des toilettes suit ici la convention de
dénominateur de l'ANSD~: il n'est calculé que sur les ménages disposant
d'une installation privée, ceux à toilettes publiques ou sans toilette
étant exclus de cet indicateur, tout en restant privés au titre du type de
toilettes. La source d'eau non améliorée retient une définition large du
traitement inadéquat~: est privé l'enfant vivant dans un ménage à source non
améliorée (à l'une ou l'autre saison) qui, soit ne traite pas du tout son
eau, soit la traite par une méthode inadéquate (filtrer à travers un linge,
laisser reposer, autre). Le dénominateur est plein (tous les ménages dont le
type de source est renseigné). Cette lecture, plus large que la seule
comptabilisation des méthodes inadéquates, situe la prévalence au-dessus de
la valeur ANSD. Le travail des enfants ne retient que les activités posant clairement le
seuil d'au moins une heure~: du côté domestique, les cinq postes d'heures du
module emploi (courses au marché, travaux domestiques, garde d'enfants ou de
personnes âgées, corvée d'eau, corvée de bois)~; du côté économique, le
travail rémunéré ou pour propre compte (questions formulées « au moins une
heure »). L'accès à une structure de santé (module communautaire) tient compte du filtre
du questionnaire~: à la question de l'existence du service (2.01), une réponse
affirmative saute la question du mode d'accès (2.02), qui n'est donc renseignée
que pour les services absents localement (trajet vers le plus proche). L'enfant
peut ainsi accéder à pied à une structure de santé si un hôpital ou un centre
de santé public existe dans sa localité (sur place) ou, à défaut, s'y rejoint à
pied~; il en est privé dans le cas contraire. L'écart avec l'ANSD tient à la
convention de dénominateur de cette dernière, qui rapporte l'indicateur aux
seules localités dépourvues de structure sur place. Le surpeuplement est
retenu dans l'agrégat mais échappe à une comparaison directe avec l'ANSD,
faute de la variable « pièces pour dormir » (calculé ici sur le nombre total de
pièces, d'où un proxy plus élevé que la valeur ANSD). Le NEET des 15-17 ans est,
lui, \emph{écarté} de l'agrégat~: sa valeur ANSD (85,7\,\%) est incohérente avec
le taux de scolarisation observé (un enfant NEET étant nécessairement non
scolarisé, alors que seuls 45,7\,\% des 15-17 ans le sont) et donc non
reproductible~; conformément au principe de ne retenir que les indicateurs
validables sur l'ANSD, la dimension Éducation des 15-17 ans repose sur le seul
illettrisme (notre NEET, estimé à 33,7\,\%, reste rapporté à titre descriptif).
La séparation parentale, troisième indicateur de la dimension Protection, est
retenue~; l'ANSD ne publie pas de taux par groupe d'âge pour cet indicateur.

Au-delà de la comparaison indicateur par indicateur, le tableau~\ref{tab:validation_globale}
confronte l'incidence N-MODA globale (nationale) obtenue dans ce mémoire à
celle publiée par l'ANSD.

\begin{table}[H]
  \centering
  \caption{Comparaison de l'incidence N-MODA globale : ce mémoire vs ANSD (EHCVM 2018, enfants 0-17 ans)}
  \label{tab:validation_globale}
  \zebre
  \begin{tabular}{|l|c|c|}
    \hline
\entete Statistique & Ce mémoire & ANSD (MODA 2024) \\
    \hline
    Incidence $H$ (\%)         & 58,5  & 50,7  \\ \hline
    Intensité $A$ (\%)         & 70,4  & 72,2  \\ \hline
    Indice ajusté $M_0 = H \times A$ & 0,412 & 0,366 \\
    \hline
  \end{tabular}

  {\footnotesize \textit{Source :} Calcul de l'auteur, EHCVM~; ANSD/UNICEF (MODA 2024) pour la colonne de comparaison.}
\end{table}

L'écart d'incidence (58,5\,\% contre 50,7\,\%) s'explique par deux facteurs.
D'une part, la nutrition est opérationnalisée de façon plus étroite que la
définition officielle~: elle ne repose ici que sur la sécurité alimentaire
(module FIES), sans le volet diversité des repas retenu par l'ANSD. D'autre
part, la dimension Santé est ici opérationnalisée de façon plus large que
dans l'agrégat de l'ANSD~: nous retenons l'union de ses deux indicateurs
officiels (combustible solide OU absence d'accès à pied à une structure de
santé), là où l'ANSD écarte le combustible de son agrégat multidimensionnel
pour manque de variance (92\,\% de privation, sans pouvoir discriminant), si
bien que sa dimension Santé se réduit à l'accès aux soins. Conserver le
combustible dans l'union élève mécaniquement la prévalence de la dimension et,
partant, l'incidence globale. Le solde de l'écart tient à des différences de
traitement statistique de l'échantillon par rapport au calcul officiel.
L'intensité, elle, reste proche de la valeur officielle.
