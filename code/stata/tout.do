...
/* Source des donnees brutes :
   - "local"  : dossier Base/ du depot clone sur la machine ;
   - "github" : telechargement direct depuis le depot GitHub (public).
     Stata lit les fichiers .dta via https avec use/merge, sans copie
     locale prealable. Attention : chaque execution re-telecharge les
     bases (~300 Mo au total), prevoir une bonne connexion.
   Les sorties (temp, output, logs) restent ecrites en local dans tous
   les cas, car save ne peut pas ecrire vers une URL. */
global SOURCE "local"
