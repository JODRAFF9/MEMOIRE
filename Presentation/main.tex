...
\begin{frame}[shrink=15]{Impact des transferts sur la pauvreté multidimensionnelle}
  \begin{table}
    ...
    \begin{tabular}{lcccc}
      ...
      \midrule
      N-MODA ($k=4$, 7 dim.)  & $+0{,}073$ & (0,035) & 0,038 & Signif. à 5\,\% \\
      Alkire-Foster ($k=1/3$) & $+0{,}051$ & (0,030) & 0,097 & Signif. à 10\,\% \\
      \bottomrule
    \end{tabular}
  \end{table}
  \begin{alertblock}{Interprétation}
    La pauvreté multidimensionnelle des enfants des ménages bénéficiaires
    stables a reculé \emph{moins vite} que celle des témoins appariés sur
    2018-2021 (période COVID). Significativité fragile selon la méthode
    d'appariement : faisceau d'indices, pas d'effet réducteur.
  \end{alertblock}
  \begin{block}{Lecture}
    Mécanismes plausibles : choc COVID sur les revenus des diasporas
    (dépendance = vulnérabilité), motif assurantiel (sélection dynamique),
    coûts de l'absence parentale (dimension protection dégradée).
  \end{block}
\end{frame}
...
\begin{frame}{Effets par dimension de la pauvreté}
  ...
  \begin{block}{Décomposition des effets}
    \begin{itemize}
      \item Éducation : seule dimension au signe favorable ($-2{,}9$ pp, $p=0{,}083$).
  ...
  \begin{block}{Hétérogénéité}
    \begin{itemize}
      \item Protection de l'enfant dégradée ($+3{,}4$ pp, $p=0{,}051$) : trace de l'absence parentale.
...
\begin{frame}{Tests de robustesse}
  \begin{block}{Sensibilité des résultats}
    \begin{itemize}
      ...
      \item Direction robuste selon la méthode (k-NN, noyau, caliper) ; significativité fragile.
...
\begin{frame}{Validation des hypothèses - Synthèse}
  \begin{table}
    ...
    \begin{tabular}{c p{3.5cm} p{5.5cm} c}
      ...
      \midrule
      H1 & Réduction de l'IPM-Enfant & ATT N-MODA = $+0{,}073$ ($p=0{,}038$) & Rejetée \\[4pt]
      H2 & Hétérogénéité des effets  & Éducation favorable vs protection dégradée ; rural, 0-4 ans & Partiellement \\[4pt]
...
\begin{frame}{Conclusion générale}
  \begin{block}{Principaux apports}
    \begin{enumerate}
      ...
      \pause
      \item L'incidence N-MODA recule de 63,8\,\% (2018) à 55,9\,\% (2021) :
            amélioration réelle mais la pauvreté reste massive.
      \pause
      \item Trajectoire relative défavorable aux bénéficiaires stables (ATT $+0{,}073$, $p=0{,}038$, non robuste) :
            les transferts n'expliquent pas à eux seuls la réduction observée.
      \pause
      \item L'impact est différencié : seule la dimension éducation bénéficie
